...
try {
    ...
    if (isset($idPersona)) {
        $stmt = $pdo->prepare("
            SELECT DB_CONFIG.idDBConfig, DB_CONFIG.idDataBase, DB_CONFIG.nombreDB, DB_CONFIG.motor, DB_CONFIG.idPersona, SERVICIO.idEtapa 
            FROM DB_CONFIG
            JOIN DATA_BASE ON DB_CONFIG.idDataBase = DATA_BASE.idDataBase
            JOIN USUARIO ON DB_CONFIG.idPersona = USUARIO.idUsuario 
            JOIN SERVICIO ON DATA_BASE.idDataBase = SERVICIO.idServicio WHERE DB_CONFIG.idPersona = :idPersona
